...
function verify_(){    //identify if the "user" is a teacher or not
  ...
  var whitelist = " gkoc8025@danbury.k12.ct.us, ajgerrish@devereuxschools.org, msco8075@danbury.k12.ct.us, createchivity@gmail.com, kiyosid378@sfxeur.com, @danbury.k12ct.us, @gmail.com" // gkoc8025@danbury.k12.ct.us is OWNER, do not delist nor should createchivity@gmail.com be delisted as that is a personal account of owner, used for other testing purposes. 5 is where to whitelist people.
  ...
  if (index != -1 && whitelist.slice(index - 1,index) == " "){ //any idea how to make verify process easier?
    ...
    return false; // In case title is lost, copy paste GoGo v1.3 Pro+ {FORMATTED EDITION}
  }}
function doGet() {
  if (verify_()){
    console.info(Session.getActiveUser());  //log your user so you know who used this
    return HtmlService.createHtmlOutputFromFile("Bypasser"); // Need to get user (self) gkoc8025@danbury.k12.ct.us authorized|check line number 5, seems to be how. (NOTE: USER HAS BEEN WHITELISTED)
  }else{
